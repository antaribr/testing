...
if s1 >= 0 and s2 >= 0:
  ...
  const badgesGridHtml = badges.length
    ? badges.map(b => {
        ...
        return `
          <div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:8px; padding:${badgeConfig.padding}; text-align:center; display:flex; flex-direction:column; align-items:center; justify-content:center; box-sizing:border-box;">
            ${logoImg}
  ...
  container.innerHTML = `
    <div>
      <!-- MAIN PAGE CARDS CONTAINER (Matching Sample PDF 100%) -->
      <div style="margin-bottom: 24px; box-sizing: border-box;">
        
        <!-- TOP HEADER BRANDING (Logo LEFT, Date RIGHT) -->
        ...
        <div class="pdf-section-card" style="background:#eef2ff; border-radius:10px; padding:12px 16px; display:flex; align-items:center; gap:14px; margin-bottom: 14px;">
          ...
          <div style="text-align:left;">
            <h2 style="margin:0; font-size:18px; font-weight:800; color:#0f172a; font-family:'Outfit', 'Inter', sans-serif;">${escapeHtml(m.fullName || 'Member Profile')}</h2>
        ...
        <div class="pdf-section-card" style="border:1px solid #e2e8f0; border-radius:10px; padding:12px 14px; background:#fafafa; margin-bottom: 24px;">
...
'''
    ...
    with open(filePathApp, 'w', encoding='utf-8') as f:
        f.write(appCode)
    print('Successfully matched PDF layout 100% to sample PDF image!')
else:
