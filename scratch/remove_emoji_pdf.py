...
# Replace emoji in app.js
appCode = appCode.replace('🎯 <strong>التكريس:</strong>', '<strong>التكريس:</strong>')
appCode = appCode.replace('🚶 <strong>الرحيل:</strong>', '<strong>الرحيل:</strong>')

with open(filePathApp, 'w', encoding='utf-8') as f:
    f.write(appCode)

# Now update mobile/register/index.html PDF generator
...
regHtml = regHtml[:s1] + pdf_exact_js.strip() + '\n\n      ' + regHtml[s2:]
