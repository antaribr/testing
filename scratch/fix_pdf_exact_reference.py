...
# 1. Update downloadMemberProfilePdf layout to match the reference image 100%
...
if s1 >= 0 and s2 >= 0:
  ...
  container.innerHTML = `
    <div>
      <!-- MAIN PAGE CARDS CONTAINER (No forced height gap) -->
      <div style="margin-bottom: 16px; box-sizing: border-box;">
        ...
        <!-- BANNER PROFILE CARD (Left-aligned Avatar + Name + Unit, matching reference image) -->
        <div class="pdf-section-card" style="background:#eef2ff; border-radius:10px; padding:12px 16px; display:flex; align-items:center; gap:14px; margin-bottom: 14px;">
          <div style="width:46px; height:46px; border-radius:50%; background:#6366f1; color:#ffffff; display:flex; align-items:center; justify-content:center; font-weight:800; font-size:18px; flex-shrink:0;">
            ${escapeHtml(initials)}
          </div>
          <div style="text-align:left;">
            <h2 style="margin:0; font-size:18px; font-weight:800; color:#0f172a; font-family:'Outfit', 'Cairo', sans-serif;">${escapeHtml(m.fullName || 'Member Profile')}</h2>
        ...
        <div class="pdf-section-card" style="border:1px solid #e2e8f0; border-radius:10px; padding:12px 14px; margin-bottom: 14px; background:#fafafa;">
        ...
        <!-- BADGES CARD SECTION (Fits compactly without artificial bottom gap) -->
        <div class="pdf-section-card" style="border:1px solid #e2e8f0; border-radius:10px; padding:12px 14px; background:#fafafa; margin-bottom: 14px;">
        ...
        <!-- ARABIC MILESTONE CARD SECTION (Right-aligned Title & RTL Timeline) -->
...
'''
    ...
    with open(filePathApp, 'w', encoding='utf-8') as f:
        f.write(appCode)
    print('Successfully matched PDF template 100% to reference screenshot!')
else:
