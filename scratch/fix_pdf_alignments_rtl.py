...
if s1 >= 0 and s2 >= 0:
  ...
  const p1MinHeightStyle = isSinglePage ? 'margin-bottom: 12px; box-sizing: border-box;' : 'min-height: 1010px; display: flex; flex-direction: column; justify-content: space-between; margin-bottom: 24px; box-sizing: border-box;';

  container.innerHTML = `
    <div>
      <!-- MAIN PAGE / CARDS CONTAINER -->
      <div style="${p1MinHeightStyle}">
        <div>
          <!-- TOP HEADER BRANDING -->
          ...
          <!-- BANNER PROFILE CARD (RTL aligned for Arabic/bilingual profile) -->
          <div class="pdf-section-card" style="background:#eef2ff; border-radius:10px; padding:12px 16px; display:flex; align-items:center; justify-content:flex-start; gap:14px; margin-bottom: 12px;" dir="rtl">
            ...
            <div style="text-align:right;">
              ...
              <div style="display:inline-block; margin-top:3px; padding:2px 8px; background:#6366f1; color:#ffffff; border-radius:5px; font-weight:800; font-size:10px; text-transform:uppercase; letter-spacing:.04em;">
        ...
        <!-- BADGES CARD SECTION -->
        <div class="pdf-section-card" style="border:1px solid #e2e8f0; border-radius:10px; padding:12px 14px; background:#fafafa; ${isSinglePage ? 'margin-bottom:12px;' : 'flex:1; display:flex; flex-direction:column; justify-content:flex-start;'}">
...
'''
    ...
    with open(filePathApp, 'w', encoding='utf-8') as f:
        f.write(appCode)
    print('Successfully updated PDF card alignments!')
else:
    print('Could not find downloadMemberProfilePdf boundary in app.js')
